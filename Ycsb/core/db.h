...
class DB {
 public:
  ...
  virtual int Read(const std::string &table, uint64_t key,
                   const std::vector<std::string> *fields,
                   std::vector<KVPair> &result) {
    // Assume 8B keys only
    thread_local std::string sbuf(4096, '\0');
    *reinterpret_cast<uint64_t *>(sbuf.data()) = key;
    return Read(table, sbuf, fields, result);
  }
  ///
  ...
  /// @param commit Set to true to commit the previous inserts, otherwise the records
  ///        are not yet persisted.
  /// @return Zero on success, a non-zero error code on error.
  ///
  virtual int Insert(const std::string &table, const std::string &key,
                     std::vector<KVPair> &values) = 0;
  virtual int Insert(const std::string &table, uint64_t key,
                     std::vector<KVPair> &values) {
    // Assume 8B keys only
    thread_local std::string sbuf(4096, '\0');
    *reinterpret_cast<uint64_t *>(sbuf.data()) = key;
    return Insert(table, sbuf, values);
  }
  ///
  ...
  virtual void thread_init(int thread_id){}
  virtual void thread_deinit(int thread_id){}
};
